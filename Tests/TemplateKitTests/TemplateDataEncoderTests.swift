...
class TemplateDataEncoderTests: XCTestCase {
    func testMustacheRenderer() throws {
        struct User: Codable {
            var name: String
            var age: Int
            var pet: Pet
        ...
        let user = User(name: "Tanner", age: 24, pet: Pet(name: "Ziz", age: 5))
        ...
        let mustache = MustacheRenderer(
            relativeDirectory: "/" + #file.split(separator: "/").dropLast().joined(separator: "/") + "/",
            fileIO: fileIO,
            on: eventLoop.next()
        )
        let view = try! mustache.render("user", user).wait()
        print(view)
...
//    }
    ...
    static var allTests = [
        ("testMustacheRenderer", testMustacheRenderer),
